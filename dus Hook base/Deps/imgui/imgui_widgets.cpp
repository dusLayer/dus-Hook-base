...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    float height = GetFrameHeight() * 0.75f;
    ...
    ImRect total_bb(
        pos,
        pos + ImVec2(
            width + (label_size.x > 0 ? style.ItemInnerSpacing.x + label_size.x : 0),
            height + 4.0f
        )
    );

    ItemSize(total_bb, 2.0f);
    ...
    float speed = 10.0f;
    anim = ImLerp(anim, target, 1.0f - expf(-speed * ImGui::GetIO().DeltaTime));
    storage->SetFloat(id, anim);

   
    ImVec4 off_col = ImVec4(0.15f, 0.15f, 0.15f, 1.0f);
    ImVec4 on_col = ImVec4(0.85f, 0.85f, 0.85f, 1.0f);
    ImVec4 bg_col = ImLerp(off_col, on_col, anim);
    ...
    float knob_w = height * 0.55f;
    float knob_h = height * 0.55f;

    float x = pos.x + anim * (width - knob_w);
    float y = pos.y + (height - knob_h) * 0.5f;
    ...
    if (*v)
    {
        window->DrawList->AddRectFilled(
            ImVec2(x - 1, y - 1),
            ImVec2(x + knob_w + 1, y + knob_h + 1),
            GetColorU32(ImVec4(1, 1, 1, 0.08f)),
            1.0f
        );
    }
    ...
    window->DrawList->AddRectFilled(
        ImVec2(x, y),
        ImVec2(x + knob_w, y + knob_h),
        GetColorU32(ImVec4(1, 1, 1, 1.0f)),
        1.5f
    );
    ...
    if (label_size.x > 0.0f)
    {
        ImVec2 label_pos = ImVec2(
            pos.x + width + style.ItemInnerSpacing.x + 2.0f,
